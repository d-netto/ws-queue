...
void vn_cl_deque_push(vn_cl_deque_t *dq, void *elt)
{
    ...
    void **buffer = READ_BUF(atomic_load_explicit(&dq->buffer, memory_order_relaxed));
    size_t capacity = READ_CAPACITY(atomic_load_explicit(&dq->size, memory_order_relaxed));
    if (b - t > capacity - 1) {
...
void **vn_cl_resize(vn_cl_deque_t *dq)
{
    ...
    // array_t *old_a = atomic_load_explicit(&dq->buffer, memory_order_relaxed);
    // array_t *new_a = create_array(2 * old_a->capacity);
    // atomic_store_explicit(&dq->buffer, new_a, memory_order_release);
...
void *vn_cl_deque_steal_from(vn_cl_deque_t *dq)
{
    ...
    if (t < b) {
        void **buffer = READ_BUF(atomic_load_explicit(&dq->buffer, memory_order_relaxed));
        size_t capacity =
            READ_CAPACITY(atomic_load_explicit(&dq->size, memory_order_relaxed));
        if (!VN_MATCH(buffer, size))
            return NULL;
